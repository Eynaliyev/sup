...
export class CreateProfilePage {
	...
	private bioValidator: ValidatorFn = Validators.compose([Validators.required]);
	...
	onInput(username: string) {
		...
		if (
			...
			!this.profileForm.controls.username.hasError("required")
		) {
			this.firestore
			...
			if (this.uniqueUsername) {
				...
				this.firestore
					...
					.then(ref => {
						...
						let firstName =
							this.profileForm.value["firstName"].charAt(0).toUpperCase() +
							this.profileForm.value["firstName"].slice(1).toLowerCase();
						let lastName =
							this.profileForm.value["lastName"].charAt(0).toUpperCase() +
							this.profileForm.value["lastName"].slice(1).toLowerCase();
						let user = new User(
							this.userId,
							this.profileForm.value["email"].toLowerCase(),
							firstName,
							lastName,
							this.photo,
							"@" + this.profileForm.value["username"].toLowerCase(),
